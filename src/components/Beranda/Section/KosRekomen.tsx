...
export default function KosRekomen() {
  const [kosRekomen, setKosRekomen] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchKosRekomen() {
      ...
      if (error) {
        console.error("Error fetching rekomendasi kos:", error);
      } else {
        setKosRekomen(
          data.map((kos) => ({
            ...kos,
    ...
    <div className="w-full px-4 md:px-6 lg:px-10">
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        <div className="flex justify-between items-center">
          <h3 className="text-primary-500 text-lg font-semibold">Kos Rekomen</h3>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Tablet & Desktop - pakai flex dan KosCard */}
